...
window.GraphDrawer = function(canvasID, rawData){
  ...
  var high = this.Max(h.slice(0,Math.min(h.length, (width-leftMargin-rightMargin) / pixels)));
  var low = this.Min(l.slice(0,Math.min(l.length, (width-leftMargin-rightMargin) / pixels)));
...
var y2=0;
  ...
  for (var i=0; i<c.length && i<(width-leftMargin-rightMargin-pixels)/pixels; i++){
    var y1 = scale(low,high,height, topMargin,bottomMargin, c[i]);
