...
window.GraphDrawer = function(canvasID, rawData){
...
*/
  ...
  var hh = this.Max(h.slice(0,Math.min(h.length, (width-leftMargin-rightMargin) / pixels)));
  var ll = this.Min(l.slice(0,Math.min(l.length, (width-leftMargin-rightMargin) / pixels)));
  // improve hh, ll
  var range = hh-ll;
  var step = 1;
  while (range/step > 16){
...
var y=0;
  ...
  for (var i=0; i<c.length && i<(width-leftMargin-rightMargin-pixels)/pixels; i++){
    var yc = scale(ll,hh,height, topMargin,bottomMargin, c[i]), x0 = (width-rightMargin) - (i+1)*pixels;
